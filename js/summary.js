var sliderValue, sectionSize, toggleOff = true;
...
function load(){
	...
	d3.json("data/summaryAllSessionCedric.json", function(error, dataAll) {
		if(error){
			...
			d3.select('#sliderByArea')
			  .call(d3.slider()
						...
						.on("slide", function(evt, value) {
								...
								if(toggleOff){
									dataExtent = d3.extent(data, function(d){ return round(parseInt(d.x),2);});
									sectionWidth = width/sliderValue;
									d3.select('#areaText').text("x range: "+round(((dataExtent[1]-dataExtent[0])/sliderValue),2));
									...
									filteredData = data.filter(isPositiveOrNegative);
									totalOfEvents = filteredData.length;
									console.log("total positive and negative events: "+filteredData.filter(isSession1).length);
									console.log("total pos: "+filteredData.filter(isSession1).filter(isPositive).length+" ,total neg: "+filteredData.filter(isSession1).filter(isNegative).length);
									eventSliderVal = Math.round(totalOfEvents/sliderValue);
									
									d3.select('#areaText').text(eventSliderVal+" events/section");
			...
			// Draw Slider by number of events
			// Only positive and negative events
			/*filteredData = data.filter(isPositiveOrNegative);
			totalOfEvents = filteredData.length;
			d3.select('#sliderByNoOfEvent')
			  .call(d3.slider()
						.axis(d3.svg.axis().ticks(6))
						.min(0)
						.max(totalOfEvents)
						.on("slide", function(evt, value) {
								eventSliderVal = Math.round(value);
								d3.select('#eventText').text(eventSliderVal);
...
function initXAxis(){
	...
	trminX = getMinY(sliderValue),
	trmaxX = getMaxY(sliderValue);
...
// return the maximum x for all section
function getMaxY(maxVal){
...
// Function to check if it's session 1
function isSession1(element) {
	return element.sessionName == "1";
}